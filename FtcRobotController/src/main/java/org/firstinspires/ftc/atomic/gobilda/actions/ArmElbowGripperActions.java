...
//    }
    public void driveByInchesArm(LinearOpMode opMode, int inches, int direction, double power){


        int ticksToReachTarget;

        if (direction == MotorConstants.DIRECTION_UP || direction == MotorConstants.DIRECTION_DOWN){

            ticksToReachTarget = (int) Math.round(inches * MotorConstants.TICKS_PER_INCH_ARM);
        } else{
        ...
        telemetry.addData("left_front current position: ", "" + armMotor.getCurrentPosition());
        telemetry.update();
        opMode.sleep((long)(1000 * 3));
        ...
        if (direction == MotorConstants.DIRECTION_UP){

            armMotor.setTargetPosition(armMotor.getCurrentPosition() + ticksToReachTarget);


        } else if (direction == MotorConstants.DIRECTION_DOWN){
